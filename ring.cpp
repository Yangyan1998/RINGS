#include <graphics.h>       // c语言 图形库
#include <conio.h>          //通过控制台输入输出函数（如按键盘操作）
#include <windows.h>        // 采用windows里的资源
...
void CleanDouble_type_one(int clean_color)	//消除第一行和第一列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music1", NULL, 0, NULL);
...
void CleanDouble_type_two(int clean_color)	//消除第一行和第二列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music2", NULL, 0, NULL);
...
void CleanDouble_type_three(int clean_color)	//消除第一行和第三列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music3", NULL, 0, NULL);
...
void CleanDouble_type_four(int clean_color)	//消除第二行和第一列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music4", NULL, 0, NULL);
...
void CleanDouble_type_five(int clean_color)	//消除第二行和第二列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music5", NULL, 0, NULL);
...
void CleanDouble_type_six(int clean_color)	//消除第二行和第三列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music6", NULL, 0, NULL);
...
void CleanDouble_type_seven(int clean_color)	//消除第三行和第一列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music7", NULL, 0, NULL);
...
void CleanDouble_type_eight(int clean_color)	//消除第三行和第二列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music8", NULL, 0, NULL);
...
void CleanDouble_type_nine(int clean_color)	//消除第三行和第三列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias music9", NULL, 0, NULL);
...
void Clear_FullPoint()	//消除同一点上三个圈颜色相同的环
{
	for (i=1;i<=9;i++)
	{
		if ((p[i].getcircle[0]==1 &&p[i].getcircle[1]==1 &&p[i].getcircle[2]==1) &&(p[i].getcolor[0]==1 &&p[i].getcolor[1]==1 &&p[i].getcolor[2]==1) )//同一点三个圆环颜色为红色
		{
			...
			mciSendString("open D:\\ringss\\xtxiaochu.mp3 alias xcmusic1", NULL, 0, NULL);
...
void CleanHeng_1(int clean_color)	//消除第一行
{
 ...
 mciSendString("stop xcmusic1", NULL, 0, NULL);   // 先把前面一次的音乐停止
mciSendString("close xcmusic1", NULL, 0, NULL); // 先把前面一次的音乐关闭
mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic1", NULL, 0, NULL);
mciSendString("play xcmusic1", NULL, 0, NULL); // 仅播放一次
 }
...
mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic2", NULL, 0, NULL);
...
mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic3", NULL, 0, NULL);
...
void CleanShu_1(int clean_color)	//消除第一列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic4", NULL, 0, NULL);
...
void CleanShu_2(int clean_color)	//消除第二列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic5", NULL, 0, NULL);
...
void CleanShu_3(int clean_color)	//消除第三列
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic6", NULL, 0, NULL);
...
void CleanXie_left(int clean_color)	//消除左上到右下颜色相同的环
{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic7", NULL, 0, NULL);
...

{
	...
	mciSendString("open D:\\ringss\\xiaochu.mp3 alias xcmusic8", NULL, 0, NULL);
...
{	
	...
	score=0;
		...
		// 定义 IMAGE 对象
	loadimage(&img_bk, "D:\\ringss\\封面.jpg");	// 读取图片到 img 对象中
...
if (c[1].x==-100 &&c[2].x==-100 &&c[3].x==-100 )
{
	...
	mciSendString("open D:\\ringss\\chuxian.mp3 alias cxmusic", NULL, 0, NULL);
	...
	if(c[1].get==1)	//当鼠标按下后圆环跟着鼠标移动
	{
		...
		for (i=1;i<=9;i++)
		{
			if(m.uMsg == WM_LBUTTONUP && m.x>=p[i].x-size && m.x<=p[i].x+size && m.y>=p[i].y-size && m.y<=p[i].y+size)
			{
				...
				mciSendString("open D:\\ringss\\fanhui.mp3 alias fhmusic1", NULL, 0, NULL);
			...
			for (i=1;i<=9;i++)
			{
				if(m.uMsg == WM_LBUTTONUP && m.x>=p[i].x-size && m.x<=p[i].x+size && m.y>=p[i].y-size && m.y<=p[i].y+size)
				{
					...
					else
					{
						...
						mciSendString("open D:\\ringss\\fanhui.mp3 alias fhmusic2", NULL, 0, NULL);
			...
			for (i=1;i<=9;i++)
			{
				...
				{ 
					...
					else
					{
						...
						mciSendString("open D:\\ringss\\fanhui.mp3 alias fhmusic3", NULL, 0, NULL);
...
void gaming()
{
		...
		mciSendString("open D:\\ringss\\bkmusic.mp3 alias bkmusic", NULL, 0, NULL);//背景音乐
